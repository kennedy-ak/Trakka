    ...
    def handle(self, *args, **options):
        ...
        for user_data in test_users:
            ...
            UserProfile.objects.create(
                user=user, role=role, department=user_data["department"]
            )

            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Created {role}: {username} (password: {password})"
                )
            )
        ...
        for user_data in test_users:
            self.stdout.write(
                f"  {user_data['role']:8} | Username: "
                f"{user_data['username']:12} | Password: "
