...
from .models import Project, TimeEntry, UserProfile


def _time_choices():
    """Generate 30-minute interval time choices from 12:00 AM to 11:30 PM"""
    choices = [("", "-- Select --")]
    for hour in range(24):
        for minute in (0, 30):
            val = f"{hour:02d}:{minute:02d}"
            t = time(hour, minute)
...
class TimeEntryForm(forms.ModelForm):
    ...
    start_time_input = forms.ChoiceField(
        choices=TIME_CHOICES,
        label="Start Time",
        widget=forms.Select(attrs={"class": "form-select"}),
    )
    end_time_input = forms.ChoiceField(
        choices=TIME_CHOICES,
        label="End Time",
        widget=forms.Select(attrs={"class": "form-select"}),
    )
    ...
    class Meta:
        model = TimeEntry
        fields = ["project", "date", "description"]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Pre-populate time fields when editing an existing entry
        if self.instance and self.instance.pk and self.instance.start_time:
            self.fields["start_time_input"].initial = self.instance.start_time.strftime(
                "%H:%M"
            )
        if self.instance and self.instance.pk and self.instance.end_time:
            self.fields["end_time_input"].initial = self.instance.end_time.strftime(
                "%H:%M"
            )

    def clean(self):
        cleaned_data = super().clean()
        start_str = cleaned_data.get("start_time_input")
        end_str = cleaned_data.get("end_time_input")
        ...
        if start_str and end_str:
            ...
            cleaned_data["computed_duration_minutes"] = int(diff.total_seconds() / 60)
            cleaned_data["parsed_start_time"] = start
            cleaned_data["parsed_end_time"] = end
...
class ProjectForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "budget_hours": forms.NumberInput(
