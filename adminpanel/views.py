...
from timesheet.forms import UserProfileForm, UserRegistrationForm
from timesheet.models import Project, TimeEntry, UserProfile, WeeklyTimesheet
...
def admin_dashboard(request):
    ...
    # Weekly timesheet statistics
    total_weekly_timesheets = WeeklyTimesheet.objects.count()
    pending_weekly_approvals = WeeklyTimesheet.objects.filter(
        status="SUBMITTED"
    ).count()
    approved_weekly_timesheets = WeeklyTimesheet.objects.filter(
        status="APPROVED"
    ).count()

    # Recent activity
    recent_users = User.objects.select_related("profile").order_by("-date_joined")[:5]
    recent_projects = Project.objects.prefetch_related("members").order_by(
        "-created_at"
    )[:5]
    ...
    total_budget_hours = (
        Project.objects.aggregate(Sum("budget_hours"))["budget_hours__sum"] or 0
    )

    # Get recent pending approvals
    pending_entries = (
        TimeEntry.objects.filter(status="PENDING")
    ...
    context = {
        ...
        "total_weekly_timesheets": total_weekly_timesheets,
        "pending_weekly_approvals": pending_weekly_approvals,
        "approved_weekly_timesheets": approved_weekly_timesheets,
...
def user_detail(request, user_id):
    ...
    profile, created = UserProfile.objects.get_or_create(
        user=user, defaults={"role": "WORKER", "department": ""}
    )
    ...
    # Annotate projects with entry counts for this user
    projects_with_counts = []
    for project in projects:
        entry_count = project.time_entries.filter(user=user).count()
        projects_with_counts.append({"project": project, "entry_count": entry_count})

    # Get recent time entries
    recent_entries = user.time_entries.select_related("project").order_by(
        "-date", "-created_at"
    )[:10]
...
def project_members_manage(request, project_id):
    ...
    project = get_object_or_404(
        Project.objects.prefetch_related("members"), id=project_id
    )
...
def reports_overview(request):
    ...
    # Status breakdown
    status_breakdown = (
        TimeEntry.objects.values("status")
        .annotate(count=Count("id"), hours=Sum("duration_minutes") / 60)
        .order_by("-count")
    )
    ...
    top_users = (
        ...
        .filter(total_hours__isnull=False)
        .order_by("-total_hours")[:10]
    )

    # Entry type breakdown
    entry_type_breakdown = TimeEntry.objects.values("entry_type").annotate(
        count=Count("id"), hours=Sum("duration_minutes") / 60
    )
